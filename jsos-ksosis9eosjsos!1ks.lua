...
task.spawn(function()
    ...
    local function loadSession()
        ...
        -- Return password even if expired, but return nil for expiration and role if it is.
        if data.password then
            if data.expiration and os.time() < data.expiration then
                return data.expiration, data.role or "Normal", data.password
            else
                -- Session expired, but we still have a password to re-validate
                return nil, nil, data.password
            end
        end

        return nil, nil, nil -- No password in session
    end
    ...
    local function InitializeMainGUI(expirationTimestamp, userRole)
        ...
        local IsEspHealthBarEnabled = false
    ...
    InfoButton.MouseButton1Click:Connect(function()
        task.spawn(function()
            local success, infoScript = pcall(function()
                return game:HttpGet("https://raw.githubusercontent.com/AREXANS/ndpeei3n3odd0d33id9dd.4nrmreoe9d3nen-dod3-emekekeekeo/refs/heads/main/info.lua")
            end)
    ...
    local GeneralTabContent = Instance.new("Frame")
    ...
    local GeneralListFrame = Instance.new("ScrollingFrame")
    ...
    local GeneralSettingsFrame = Instance.new("ScrollingFrame")
    ...
    saveFeatureStates = function()
        ...
        local statesToSave = {
            ...
            ESPLine = IsEspLineEnabled,
            ESPHealthBar = IsEspHealthBarEnabled,
    ...
    local function setupGeneralTab()
        ...
        collapseButton.MouseButton1Click:Connect(function()
            ...
            triangleIcon.Text = isMasterEnabled and "▲" or "▼"
        ...
        local walkSpeedToggle, _ = createToggle(GeneralListFrame, "Jalan Cepat", IsWalkSpeedEnabled, function(v) IsWalkSpeedEnabled = v; ToggleWalkSpeed(v) end)
        walkSpeedToggle.LayoutOrder = 4
        
        -- [[ INTEGRASI KUNCI KECEPATAN UI ]] --
        local speedLockBox = createTextBox(GeneralListFrame, "Kecepatan Terkunci", speedLock_currentSpeed, function(v) 
        ...
        local speedLockToggle, _ = createToggle(GeneralListFrame, "Kunci Kecepatan", speedLock_isEnforced, function(state)
        ...
        local flySpeedBox = createTextBox(GeneralListFrame, "Kecepatan Terbang", Settings.FlySpeed, function(v) Settings.FlySpeed = v end)
        flySpeedBox.LayoutOrder = 7
        local flyToggle, _ = createToggle(GeneralListFrame, "Terbang", IsFlying, function(v) if v then if UserInputService.TouchEnabled then StartMobileFly() else StartFly() end else if UserInputService.TouchEnabled then StopMobileFly() else StopFly() end end end)
        flyToggle.LayoutOrder = 8
        local noclipToggle, _ = createToggle(GeneralListFrame, "Noclip", IsNoclipEnabled, function(v) ToggleNoclip(v) end)
        noclipToggle.LayoutOrder = 9
        local infJumpToggle, _ = createToggle(GeneralListFrame, "Infinity Jump", IsInfinityJumpEnabled, function(v) IsInfinityJumpEnabled = v; saveFeatureStates(); if v then if LocalPlayer.Character and LocalPlayer.Character.Humanoid then infinityJumpConnection = ConnectEvent(UserInputService.JumpRequest, function() LocalPlayer.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Jumping) end) end elseif infinityJumpConnection then infinityJumpConnection:Disconnect(); infinityJumpConnection = nil end end)
        infJumpToggle.LayoutOrder = 10
        local godModeToggle, _ = createToggle(GeneralListFrame, "Mode Kebal", IsGodModeEnabled, ToggleGodMode)
        godModeToggle.LayoutOrder = 11
        local feInvisToggle, _ = createToggle(GeneralListFrame, "FE Invisible", IsFEInvisibleEnabled, ToggleFEInvisible)
        feInvisToggle.LayoutOrder = 12
        local invisSlider = createSlider(GeneralListFrame, "Transparansi Invisible", 0, 100, Settings.FEInvisibleTransparency * 100, "%", 1, function(v)
        ...
        local touchFlingButton = createButton(GeneralListFrame, "Buka Touch Fling", CreateTouchFlingGUI)
        touchFlingButton.LayoutOrder = 14
        local antiFlingToggle, _ = createToggle(GeneralListFrame, "Anti-Fling", antifling_enabled, ToggleAntiFling)
        antiFlingToggle.LayoutOrder = 15
        local magnetButton = createButton(GeneralListFrame, "Buka GUI Magnet", createMagnetGUI)
        magnetButton.LayoutOrder = 16
        local partControllerButton = createButton(GeneralListFrame, "Buka GUI Part Controller", createPartControllerGUI)
        partControllerButton.LayoutOrder = 17
    end
    ...
    setupSettingsTab = function()
        ...
        createToggle(SettingsTabContent, "Anti AFK", IsAntiAFKEnabled, ToggleAntiAFK).LayoutOrder = 8
        ...
        themeOptionsContainer.LayoutOrder = 9
        ...
        createButton(SettingsTabContent, "Simpan Posisi UI", saveGuiPositions).LayoutOrder = 10
        createButton(SettingsTabContent, "Hop Server", function() HopServer() end).LayoutOrder = 11
    ...
    setupRekamanTab = function()
        ...
        cleanupSinglePlayback = function(isSequence)
            ...
            -- Disable movers before destroying to release control smoothly
            if playbackMovers.alignPos then pcall(function() playbackMovers.alignPos.Enabled = false end) end
            if playbackMovers.alignOrient then pcall(function() playbackMovers.alignOrient.Enabled = false end) end
            task.wait(0.05) -- Wait a couple frames for physics to settle

            -- Destroy all playback-related instances
            for _, mover in pairs(playbackMovers) do
                if mover and mover.Parent then pcall(function() mover:Destroy() end) end
            end
            playbackMovers = {}
            ...
            -- Clean up sound
            if hrp then
                local customSound = hrp:FindFirstChild("CustomRunningSound")
                if customSound then pcall(function() customSound:Destroy() end) end
            end
            ...
            -- Force physics state reset and return network ownership
            if hrp then
                pcall(function()
                    hrp.Velocity = Vector3.new(0, 0, 0)
                    hrp.RotVelocity = Vector3.new(0, 0, 0)
                    hrp.Anchored = false
                    hrp:SetNetworkOwner(nil) -- Return control explicitly
                end)
            end
            
            -- Force the humanoid into a controllable state
        ...
        playSequence = function(replayCountBox)
            ...
            local function playNextInSequence()
                ...
                if currentSequenceIndex > #sequenceToPlay then
                    ...
                    -- Reset state for smooth looping by performing a full cleanup
                    cleanupSinglePlayback(false) 
                    task.wait(0.2) -- Short delay to ensure full reset
...
local RECORDING_EXPORT_FILE = RECORDING_FOLDER .. "/" .. exportName .. ".json"
    ...
    setupTeleportTab()
    setupVipTab()
    local function setupEspColorSettings()
        ...
        for _, data in ipairs(passwordData) do
            if data.password == password then
                ...
                if newExpiration and os.time() < newExpiration then
                    ...
                    -- Selalu simpan sesi jika valid, untuk memperbarui tanggal kedaluwarsa jika berubah.
    ...
    if savedPassword then
        -- Selalu coba validasi ulang kata sandi yang tersimpan dengan server
        ...
        if newExpiration and newRole then
            ...
            if success and passwordData then
                ...
                warn("Tidak dapat mengambil file kata sandi untuk login baru.", passwordData)
            end
        end
    else
